...
namespace Server
{
    ...
    class StartServer
    {
        static void Main(string[] args)
        {
            ...
            ProtoV3ChatServer server = new ProtoV3ChatServer("127.0.0.1", 55556, serviceImpl);
            server.Start();
    ...
    {
        private ITicketSystemServices server;
        private protobuf.ProtoTSWorker worker;
        public ProtoV3ChatServer(string host, int port, ITicketSystemServices server)
            : base(host, port)
        {
            this.server = server;
            Console.WriteLine("ProtoTSServer...");
        }
        protected override Thread createWorker(TcpClient client)
        {
            worker = new protobuf.ProtoTSWorker(server, client);
            return new Thread(new ThreadStart(worker.run));
        }
    }
